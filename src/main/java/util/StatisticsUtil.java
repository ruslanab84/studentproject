    ...
    public static List<Statistics> createStatistics(List<Student> students,
        ...
        profiles.forEach(profile -> {
            ...
            List<Student> profileStudents = students.stream()
                    ...
                    .collect(Collectors.toList());
            statistics.setStudentCountProfile(profileStudents.size());
            OptionalDouble avgExamScore = profileStudents.stream()
                    .mapToDouble(Student::getAvgExamScore)
                    .average();
            statistics.setAvgExamPoint(0);
            avgExamScore.ifPresent(value -> statistics.setAvgExamPoint(
                    (int) BigDecimal.valueOf(value).setScale(2, RoundingMode.HALF_UP).doubleValue()));
        });
